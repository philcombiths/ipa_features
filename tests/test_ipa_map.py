import pytest
from ipa_features.ipa_map import (
    ipa_parser,
    segment_generator,
    PhoElement,
    PhoBase,
...
def test_ipa_parser_simple():
    ...
    # Test basic segment
    assert ipa_parser("pʰ") == [[PhoElement("p"), PhoElement("ʰ")]]
...
# segment3 = PhoSegment(segment3_components) # Compounds not yet implemented
...
def test_phosegment_base_features():
    assert segment1.get_feature("voice") == PhoElement("t").classify().features['Voice']
    assert segment1.get_feature("voice") == 0
    assert segment2.get_feature("place") == PhoElement("h").classify().features['Place']
    assert segment2.get_feature("place") == 'glottal'
    
def test_phosegment_combined_features():
